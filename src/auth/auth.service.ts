...
export class AuthService {
  ...
  async signup(
    userInfo: SignupUserDto,
    authMailToken: string,
  ): Promise<boolean> {
    const isExistSameEmail: boolean = await this.userService.checkSameEmail(
      userInfo.e_mail,
    );
    if (isExistSameEmail) return false;
    const salt: string = bcrypt.genSaltSync();
    const password: string = bcrypt.hashSync(userInfo.password, salt);

    const createdUser: User | boolean = await this.usersModel
      .create({
        ...userInfo,
        password,
        authMailToken,
